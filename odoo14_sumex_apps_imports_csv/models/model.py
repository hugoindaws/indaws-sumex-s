...
class sumex_apps_imports_csv(models.Model):
	...
	import_names = [
		('sumex_apps_imports_csv_photostock_productos', 'PhotoStock - Productos'),
		('sumex_apps_imports_csv_importador_clientes_de_sage', 'Sage - Clientes'),
		('sumex_apps_imports_csv_importador_pedidos_cabeceras_de_sage', 'Sage - Pedidos Cabeceras'),
		('sumex_apps_imports_csv_importador_pedidos_lineas_de_sage', 'Sage - Pedidos Lineas'),
	]
	...
	def test_now_action(self):
		...
		return self._import_now(True)

	def import_now_action(self):

		return self._import_now()
	...
	def _import_now(self, test = False):
		...
		list_csv_header_fields_positions = self._get_and_test_csv_header(file_csv_header, import_obj.import_fields)
		...
		row = self.env['sumex_apps_imports_csv'].sudo().search([['id', '=', self.id]])
		modo = "ejecutando"
		if test:
			modo = "testeando"
		self._update({
		...
		if hasattr(import_obj, 'hook_pre_process'):
			import_obj.hook_pre_process(row.company_id.id, file_csv_header, file_csv_content)
		...
		self._progress(row, file_csv_content_total_rows, file_csv_content_row_num, True)

		for file_csv_content_row in file_csv_content:

			self._progress(row, file_csv_content_total_rows, file_csv_content_row_num, False)
			...
			file_csv_content_row_num += 1
			file_csv_content_row_dict = self._get_csv_row_as_dict_with_names(list_csv_header_fields_positions, file_csv_content_row)  # Reestructurar la fila con formato {columna_nombre: valor, ...}

			#  Validar la fila con los campos requeridos denidos en el importador
			validate_row = self._validate_csv_row(import_obj, file_csv_content_row_dict)
			validate_result = self._validate_result(row, validate_row, file_csv_content_row_num)
			if not validate_result:
				continue

			# Validaciones del importador
			validate_row = import_obj.validate_row(file_csv_content_row_num, row.company_id.id, file_csv_content_row_dict)
			validate_result = self._validate_result(row, validate_row, file_csv_content_row_num)
			if not validate_result:
				continue
			...
			if not test:
				...
				result = import_obj.import_row(file_csv_content_row_num, row.company_id.id, file_csv_header, file_csv_content_row_dict)

				# self._validate_result(row, result, file_csv_content_row_num)
		...
		if hasattr(import_obj, 'hook_post_process'):
			import_obj.hook_post_process(row.company_id.id, file_csv_header, file_csv_content_row_dict, file_csv_content)
	...
	def _validate_csv_row(self, import_obj, file_csv_content_row):

		if not file_csv_content_row:
			return {'error': "Fila vacía"}

		for import_fields_item in import_obj.import_fields:
			# validar si existe el valor en la columna requerida
			fieldname = import_fields_item['csv_column_name']
			required_field = import_fields_item['required']
			if required_field is True:
				if fieldname not in file_csv_content_row or not file_csv_content_row[fieldname]:
					return {'error': "En esta fila la columna '%s' no tiene valor" % fieldname}
		return True

	def _validate_result(self, row, result, file_csv_content_row_num):
	...
	def _progress(self, row, total, value, force = False):

		pass_time = process_time() - float(row.last_progress_commit_time)
		if not force and pass_time < 5:
			return
		...
		for import_fields_item in import_fields:
			...
			for file_csv_header_item in file_csv_header:
				if not file_csv_header_item:
					pass
				i += 1
				if file_csv_header_item == import_fields_item['csv_column_name']:
					found = True
					list_csv_header_fields_positions[i] = file_csv_header_item
					break
			if import_fields_item['required'] is True and not found:
				self._update({'state': "finerror"})
				self._insert_log(self.id, 'error', False, 'csv file has no header text %s' % file_csv_header_item[0])
				return False
	...
	def _get_csv_row_as_dict_with_names(self, list_csv_header_fields_positions, file_csv_content_row):
		...
		for file_csv_content_row_value in sanitized_file_csv_content_row:
			...
			if i in list_csv_header_fields_positions:
				field_name = list_csv_header_fields_positions[i]
				file_csv_content_row_list[field_name] = file_csv_content_row_value

		# rellenar los campos faltantes en list_csv_header_fields_positions con valor False
		for list_csv_header_fields_positions_idx in list_csv_header_fields_positions:
			list_csv_header_fields_positions_name = list_csv_header_fields_positions[list_csv_header_fields_positions_idx]
			if list_csv_header_fields_positions_name not in file_csv_content_row_list:
				file_csv_content_row_list[list_csv_header_fields_positions_name] = False
